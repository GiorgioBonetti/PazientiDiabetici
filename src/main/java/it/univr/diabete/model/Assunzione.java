package it.univr.diabete.model;

import java.time.LocalDateTime;

public class Assunzione {

    private int id;
    private LocalDateTime dateStamp;
    private int quantitaAssunta;
    private String fkPaziente;
    private int fkTerapia;
    private int fkFarmaco;

    public Assunzione() { }

    public Assunzione(int id, LocalDateTime dateStamp,
             int quantitaAssunta, String fkPaziente,
             int fkTerapia, int fkFarmaco) {
        this.id = id;
        this.dateStamp = dateStamp;
        this.quantitaAssunta = quantitaAssunta;
        this.fkPaziente = fkPaziente;
        this.fkTerapia = fkTerapia;
        this.fkFarmaco = fkFarmaco;
    }

    // getter & setter
    public int getId() { return id; }
    public void setId(int id) { this.id = id; }

    public LocalDateTime getDateStamp() { return dateStamp; }
    public void setDateStamp(LocalDateTime dateStamp) { this.dateStamp = dateStamp; }

    public int getQuantitaAssunta() { return quantitaAssunta; }
    public void setQuantitaAssunta(int quantitaAssunta) { this.quantitaAssunta = quantitaAssunta; }

    public String getFkPaziente() { return fkPaziente; }
    public void setFkPaziente(String fkPaziente) { this.fkPaziente = fkPaziente; }

    public int getFkTerapia() { return fkTerapia; }
    public void setFkTerapia(int fkTerapia) { this.fkTerapia = fkTerapia; }

    public int getFkFarmaco() { return fkFarmaco; }
    public void setFkFarmaco(int fkFarmaco) { this.fkFarmaco = fkFarmaco; }
}